<div id="page" class="site">
 <div id="top-bar" class="top-bar-widgets col-2">
            <div class="wrapper">

                <div class="widget widget_address_block">
                    <ul>
                        <li><i class="fas fa-map-marker-alt"></i>27, Rue de Guise Roche Noire Casablanca</li>
                        <li><a href="tel: 06 69 43 93 63"><i class="fas fa-phone-alt"></i>06 69 43 93 63</a></li>
                        <li><a href="mailto:crecheminibulle@gmail.com"><i
                                    class="far fa-envelope"></i>crecheminibulle@gmail.com</a></li>
                    </ul>
                </div><!-- .widget_address_block -->
                <div class="widget widget_social_icons">
                    <div class="social-icons">
                        <ul>
                            <li><a href="https://www.instagram.com/crecheminibulle" target="_blank"></a></li>
                            <li><a href="https://www.facebook.com/crecheminibulle" target="_blank"></a></li>
                        </ul>
                        
                        <!-- Language Menu -->
                        <div class="language-menu-container">
                            <div class="language-switcher">
                                <ul class="language-nav">
                                    <li>
                                        <a href="<?php echo LANG_BASE_PATH; ?>/fr/<?php echo $page_path; ?>" class="lang-fr" title="Français">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="frensh-icon" width="30" height="20" viewBox="0 0 3 2">
                                                <rect width="1" height="2" x="0" y="0" fill="#0055A4"/>
                                                <rect width="1" height="2" x="1" y="0" fill="#fff"/>
                                                <rect width="1" height="2" x="2" y="0" fill="#EF4135"/>
                                                </svg>

                                        </a>
                                    </li>
                                    <li class="separator">|</li>
                                    <li>
                                        <a href="<?php echo LANG_BASE_PATH; ?>/en/<?php echo $page_path; ?>" class="lang-en active" title="English">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="english-icon" viewBox="0 0 50 30" width="30" height="20">
                                            <clipPath id="t">
                                                <path d="M25,15h25v15zv15h-25zh-25v-15zv-15h25z"/>
                                            </clipPath>
                                            <path d="M0,0v30h50v-30z" fill="#012169"/>
                                            <path d="M0,0 50,30M50,0 0,30" stroke="#fff" stroke-width="6"/>
                                            <path d="M0,0 50,30M50,0 0,30" clip-path="url(#t)" stroke="#C8102E" stroke-width="4"/>
                                            <path d="M-1 11h22v-12h8v12h22v8h-22v12h-8v-12h-22z" fill="#C8102E" stroke="#FFF" stroke-width="2"/>
                                        </svg>

                                        </a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                </div><!-- .widget_social_icons -->
            </div><!-- .wrapper -->
        </div><!-- #top-bar -->
        <header id="masthead" class="site-header" role="banner">
            <div class="wrapper">
                <div class="site-branding">
                    <div class="site-logo">
                        <a href="<?php echo $nav_base; ?>index.php" class="custom-logo-link" rel="home" aria-current="page"><img width="360"
                                height="351" src="<?php echo $base_path; ?>assets/images/icon/cropped-logo-creche-minibulle.png"
                                class="custom-logo" alt="Minibulle" decoding="async" fetchpriority="high"
                                sizes="(max-width: 360px) 100vw, 360px"></a> </div><!-- .site-logo -->

                    <div id="site-identity">
                        <p class="site-title">
                            <a href="<?php echo $nav_base; ?>index.php" rel="home"> Minibulle</a>
                        </p>

                        <p class="site-description">Nursery - Daycare &#8211; Kindergarten</p>
                    </div><!-- #site-identity -->
                </div> <!-- .site-branding -->

                <nav id="site-navigation" class="main-navigation" role="navigation" aria-label="Primary Menu">
                    <button type="button" class="menu-toggle">
                        <span class="icon-bar"></span>
                        <span class="icon-bar"></span>
                        <span class="icon-bar"></span>
                    </button>

                    <div class="menu-primary-container">
                        <ul id="primary-menu" class="nav-menu">
                            <li id="menu-item-6"
                                class="menu-item menu-item-type-post_type menu-item-object-page menu-item-home menu-item-6">
                                <a href="<?php echo $nav_base; ?>index.php" >
                                    <div class="icon icon5">
                                        <svg xmlns="http://www.w3.org/2000/svg" version="1.0"  viewBox="0 0 1280.000000 1280.000000" preserveAspectRatio="xMidYMid meet">
    
                                            <g transform="translate(0.000000,1280.000000) scale(0.100000,-0.100000)" fill="#ffffffff" stroke="none">
                                            <path d="M4969 10256 c-646 -572 -1825 -1616 -2620 -2320 -794 -703 -1445 -1282 -1447 -1286 -3 -8 537 -620 548 -620 4 0 407 354 896 788 489 433 1515 1341 2278 2017 764 677 1426 1263 1472 1302 l82 73 2358 -2090 c1298 -1149 2364 -2089 2369 -2089 6 0 131 138 279 306 196 223 267 309 260 319 -5 7 -1185 1054 -2621 2327 -2305 2040 -2617 2313 -2645 2314 -30 1 -146 -100 -1209 -1041z"/>
                                            <path d="M2470 9648 l0 -992 33 30 c31 30 1280 1156 1284 1158 0 1 5 180 9 399 l7 397 -667 0 -666 0 0 -992z"/>
                                            <path d="M5744 9127 c-236 -210 -1070 -949 -1852 -1641 l-1422 -1259 0 -2371 c0 -2275 1 -2373 19 -2411 23 -51 74 -101 120 -117 28 -10 286 -13 1216 -13 l1180 0 5 1065 c5 985 6 1067 22 1097 23 42 80 88 127 102 28 8 315 11 1027 11 939 0 991 -1 1030 -19 57 -26 110 -92 123 -153 7 -33 11 -392 11 -1078 l0 -1030 1163 0 c1261 0 1218 -2 1291 54 19 14 43 45 55 69 l21 43 1 2375 1 2374 -314 280 c-289 258 -3344 2963 -3378 2991 -13 10 -83 -48 -446 -369z"/>
                                            </g>
                                        </svg>
                                    </div>
                                    Home

                                </a>
                            </li>
                            <li id="menu-item-28"
                                class="menu-item menu-item-type-post_type menu-item-object-page  menu-item-28">
                                <a href="<?php echo $nav_pages; ?>a_propos.php">
                                    <div class="icon icon1">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
                                            <path fill="#ffffffff" d="M320 312C386.3 312 440 258.3 440 192C440 125.7 386.3 72 320 72C253.7 72 200 125.7 200 192C200 258.3 253.7 312 320 312zM290.3 368C191.8 368 112 447.8 112 546.3C112 562.7 125.3 576 141.7 576L498.3 576C514.7 576 528 562.7 528 546.3C528 447.8 448.2 368 349.7 368L290.3 368z"/></svg>
                                    </div>

                                    About Us
                                </a>
                                
                                </li>
                            <li id="menu-item-460"
                                class="menu-item menu-item-type-post_type menu-item-object-page  menu-item-460">
                                <a href="<?php echo $nav_pages; ?>espace.php">
                                    <div class="icon icon2">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path fill="#ffffffff" d="M94.7 136.3C101.6 112.4 123.5 96 148.4 96L492.4 96C517.3 96 539.2 112.4 546.2 136.3L569.6 216.5C582.4 260.2 549.5 304 504 304C477.7 304 454.6 289.1 443.2 266.9C431.6 288.8 408.6 304 381.8 304C355.2 304 332.1 289 320.5 267C308.9 289 285.8 304 259.2 304C232.4 304 209.4 288.9 197.8 266.9C186.4 289 163.3 304 137 304C91.4 304 58.6 260.3 71.4 216.5L94.7 136.3zM160.4 416L480.4 416L480.4 349.6C488 351.2 495.9 352 503.9 352C518.2 352 531.9 349.4 544.4 344.8L544.4 496C544.4 522.5 522.9 544 496.4 544L144.4 544C117.9 544 96.4 522.5 96.4 496L96.4 344.8C108.9 349.4 122.5 352 136.9 352C145 352 152.8 351.2 160.4 349.6L160.4 416z"/></svg>
                                    </div>                                
                                    Our Space
                                </a>
                            </li>
                            <li id="menu-item-492"
                                class="menu-item menu-item-type-custom menu-item-object-custom menu-item-has-children menu-item-492">
                                <a href="#" style= "display: block ;">
                                    <div class="icon icon3">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path fill="#ffffffff" d="M352 128C352 110.3 337.7 96 320 96C302.3 96 288 110.3 288 128L288 288L128 288C110.3 288 96 302.3 96 320C96 337.7 110.3 352 128 352L288 352L288 512C288 529.7 302.3 544 320 544C337.7 544 352 529.7 352 512L352 352L512 352C529.7 352 544 337.7 544 320C544 302.3 529.7 288 512 288L352 288L352 128z"/></svg>
                                    </div>                                
                                    Plus
                                </a>
                                <ul class="sub-menu">
                                    <li id="menu-item-524"
                                        class="menu-item menu-item-type-post_type menu-item-object-page  menu-item-524">
                                        <a href="<?php echo $nav_pages; ?>inscription.php">Registration</a></li>
                                    <li id="menu-item-491"
                                        class="menu-item menu-item-type-post_type menu-item-object-page menu-item-491">
                                        <a href="<?php echo $nav_pages; ?>pedagogique.php">Educational Approach</a></li>
                                    <li id="menu-item-614"
                                        class="menu-item menu-item-type-post_type menu-item-object-page  menu-item-614">
                                        <a href="<?php echo $nav_pages; ?>evenements.php">Our Events</a></li>
                                    <li id="menu-item-426"
                                        class="menu-item menu-item-type-post_type menu-item-object-page menu-item-426">
                                        <a href="<?php echo $nav_pages; ?>blog.php">Blog</a></li>
                                </ul>
                            </li>
                            <li id="menu-item-30"
                                class="menu-item menu-item-type-post_type menu-item-object-page  menu-item-30">
                                <a href="<?php echo $nav_pages; ?>contact.php">
                                    <div class="icon icon4">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640"><path fill="#ffffffff" d="M112 128C85.5 128 64 149.5 64 176C64 191.1 71.1 205.3 83.2 214.4L291.2 370.4C308.3 383.2 331.7 383.2 348.8 370.4L556.8 214.4C568.9 205.3 576 191.1 576 176C576 149.5 554.5 128 528 128L112 128zM64 260L64 448C64 483.3 92.7 512 128 512L512 512C547.3 512 576 483.3 576 448L576 260L377.6 408.8C343.5 434.4 296.5 434.4 262.4 408.8L64 260z"/></svg>
                                    </div>                                
                                    Contact
                                </a>
                            </li>
                            <li id="menu-item-30" class="menu-item menu-item-type-post_type menu-item-object-page">
                                <div class=" mobile-language-menu">
                                   <a href="<?php echo LANG_BASE_PATH; ?>/fr/<?php echo $page_path; ?>" class="lang-fr" title="Français">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="frensh-icon" width="30" height="20" viewBox="0 0 3 2">
                                            <rect width="1" height="2" x="0" y="0" fill="#0055A4"/>
                                            <rect width="1" height="2" x="1" y="0" fill="#fff"/>
                                            <rect width="1" height="2" x="2" y="0" fill="#EF4135"/>
                                            </svg>

                                    </a>
                                    <a href="<?php echo LANG_BASE_PATH; ?>/en/<?php echo $page_path; ?>" class="lang-en active" title="English">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="english-icon" viewBox="0 0 50 30" width="30" height="20">
                                            <clipPath id="t">
                                                <path d="M25,15h25v15zv15h-25zh-25v-15zv-15h25z"/>
                                            </clipPath>
                                            <path d="M0,0v30h50v-30z" fill="#012169"/>
                                            <path d="M0,0 50,30M50,0 0,30" stroke="#fff" stroke-width="6"/>
                                            <path d="M0,0 50,30M50,0 0,30" clip-path="url(#t)" stroke="#C8102E" stroke-width="4"/>
                                            <path d="M-1 11h22v-12h8v12h22v8h-22v12h-8v-12h-22z" fill="#C8102E" stroke="#FFF" stroke-width="2"/>
                                        </svg>                                              

                                    </a>
                                </div>
                            </li>
                        </ul>
                    </div>
                </nav><!-- #site-navigation -->
            </div><!-- .wrapper -->
        </header> <!-- header ends here -->